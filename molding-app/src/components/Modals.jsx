...
export function UserModal({ lang, item, onSave, onClose }) {
  ...
  const save = async () => {
    ...
    try {
      const pl = item
        ? { id: item.id, username: username.trim(), role, email, notifyReport: notify, ...(password ? { password } : {}) }
        : { username: username.trim(), password, role, email, notifyReport: notify };
  ...
  return (
    <div className="modal-bg" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-sheet">
        ...
        <div className="field"><label className="field-label">{tx(lang, 'role')}</label>
